...
| `elements.html` | Atoms (typography, buttons, inputs, ΓÇª) rendered + annotated. |
| `components.html` | Composites (cards, navbars, forms, ΓÇª) rendered + annotated. |
| `sections.html` | Page sections (hero, pricing, footer, ΓÇª) rendered + annotated. |

Each HTML file is **standalone** ΓÇö open it directly in a browser (`file://`). It loads Tailwind v4 from the Play CDN (`https://unpkg.com/@tailwindcss/browser@4`) and includes an inline `@theme` block at the top that mirrors `tokens.json`, so specimens render with the project's actual tokens.
...
**Update workflow.** When you add a specimen to an HTML file, flip the matching checkbox below from `[ ]` ΓåÆ `[~]` (partial coverage, e.g. some states still missing) or `[x]` (fully captured).
...
**Palette ramps (50ΓÇô950 per hue)**
- [x] violet ΓÇö anchors `primary` (500) and `primary-hover` (600)
- [x] teal ΓÇö anchors `secondary` (500) and `secondary-hover` (600)
- [x] amber ΓÇö anchors `tertiary` (500) and `tertiary-hover` (600)
- [ ] gray / neutral ramp (currently flat `bg` / `surface` / `fg` tokens; promote to a ramp when more shades are needed)
- [ ] semantic ramps (`success` / `warning` / `danger` / `info` ΓÇö flat today; promote if alert backgrounds need a 100/950 tint)
...
- [ ] Brand soft tints (e.g. `primary-soft` for alert backgrounds ΓÇö currently using `bg-primary/15` inline)
- [x] Accent
...
## Elements ΓÇö `elements.html`

Atoms. Single-purpose primitives.

### Typography
- [ ] h1ΓÇôh6 + lead paragraph
- [ ] Body, small, mono, kbd, code
- [ ] Lists (ul / ol)
- [ ] Blockquote

### Form atoms
- [ ] Button ΓÇö primary / secondary / ghost / destructive
- [ ] Button states ΓÇö default / hover / active / focus / disabled / loading
...
### Navigation atoms
- [ ] Link ΓÇö default / hover / visited
- [ ] Breadcrumb item
...
## Components ΓÇö `components.html`
