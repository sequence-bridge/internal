...
Invoke the `internal-scaffolding` skill whenever you start work in this project ΓÇö it handles both first-time setup and all ongoing phases (filling in steering docs, speccing features, planning, implementing, archiving). This file owns the **rules** for what lives here and how it's named.
...
```
internal/
Γö£ΓöÇΓöÇ CLAUDE.md              ΓåÉ this file
Γö£ΓöÇΓöÇ steering/              ΓåÉ stable across features; updated deliberately
Γöé   Γö£ΓöÇΓöÇ product.md         ΓåÉ vision, strategy, objectives
Γöé   Γö£ΓöÇΓöÇ roadmap.md         ΓåÉ monthly headings + backlog (no version table)
Γöé   ΓööΓöÇΓöÇ tech-stack.md      ΓåÉ languages, components, conventions, TDD policy
Γö£ΓöÇΓöÇ design/                ΓåÉ visual identity; gap-tracked + browser-viewable
Γöé   Γö£ΓöÇΓöÇ design.md          ΓåÉ ideal design system as a checklist (gap tracker)
Γöé   Γö£ΓöÇΓöÇ tokens.json        ΓåÉ design tokens, W3C DTCG format
Γöé   Γö£ΓöÇΓöÇ elements.html      ΓåÉ atoms, rendered + annotated
Γöé   Γö£ΓöÇΓöÇ components.html    ΓåÉ composites, rendered + annotated
Γöé   ΓööΓöÇΓöÇ sections.html      ΓåÉ page sections, rendered + annotated
Γö£ΓöÇΓöÇ specs/                 ΓåÉ per-feature specs, flat list
Γöé   ΓööΓöÇΓöÇ YYYY-MM-DD-<slug>.md
Γö£ΓöÇΓöÇ qa/                    ΓåÉ raw QA feedback from hands-on testing
Γöé   ΓööΓöÇΓöÇ YYYY-MM-DD-<slug>.md
Γö£ΓöÇΓöÇ content/               ΓåÉ writing style guide + per-piece drafts
Γöé   Γö£ΓöÇΓöÇ style.md           ΓåÉ voice, tone, word swaps, paired examples
Γöé   ΓööΓöÇΓöÇ YYYY-MM-DD-<slug>.md  ΓåÉ individual content pieces
ΓööΓöÇΓöÇ releases/
    Γö£ΓöÇΓöÇ CHANGELOG.md       ΓåÉ auto-populated per push (see rule below)
    ΓööΓöÇΓöÇ roadmap-log.md     ΓåÉ hand-curated, populated by the archive command
```
...
- **Releases**: fixed names (`CHANGELOG.md`, `roadmap-log.md`). No dating in the filename ΓÇö the dates live inside.
...
`internal/qa/` holds raw feedback from hands-on testing of shipped work ΓÇö bugs, things that feel wrong, reference screenshots and links. Deliberately unpolished: no template, no required sections, write it the way you'd say it.
...
Workflow when drafting a piece: read `style.md` first; pull in the relevant voice anchors and tone shift; check word swaps and house vocabulary against the draft before shipping. If a piece needs to break a rule, document the exception in `style.md` under Notes & exceptions ΓÇö don't quietly drift.
...
- `design.md` ΓÇö the **ideal** design system as a checklist. Each item is marked `[ ]` (missing), `[~]` (partial), or `[x]` (captured). Single place to see what's left to build out.
- `tokens.json` ΓÇö design tokens in [W3C DTCG](https://tr.designtokens.org/format/) format (`{ "$value", "$type" }`). The values that drive everything else.
- `elements.html`, `components.html`, `sections.html` ΓÇö standalone, browser-viewable pages. Open via `file://`. Each loads Tailwind v4 from the Play CDN (`https://unpkg.com/@tailwindcss/browser@4`) and includes an inline `@theme` block at the top that mirrors `tokens.json`, so every specimen uses the project's actual tokens.

Workflow when adding a specimen: render it in the appropriate HTML file ΓåÆ flip its checkbox in `design.md` from `[ ]` to `[~]` or `[x]`. When token values change, update `tokens.json` **and** the inline `@theme` block in each HTML file (no automatic sync until the project has a real Tailwind build pipeline; at that point, replace the CDN script + inline `@theme` with the compiled CSS).
